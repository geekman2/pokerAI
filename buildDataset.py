...
for ii,filename in enumerate(tableFiles):
    ...
    comboRanges = []
    for cards in itertools.combinations(range(4),3):
    ...
    comboRanges = []
    for cards in itertools.combinations(range(5),3):
        c = boardRanks.ix[:,cards]
        comboRanges.append((c.max(axis=1) - c.min(axis=1) == 2) & (c.notnull().sum(axis=1)==3))
    ...
    with open('Player.txt') as playerF, open('Action.txt') as actionF, \
        ...
        pla = []
        lastActions = {}
        for i,(p,aStr) in enumerate(izip(playerF, actionF)):
            if p in lastActions:
                pla.append(lastActions[p])
            else:
                pla.append(-1)
            a = actions.index(aStr.strip())
            lastActions[p] = a
            if i % 10000000 == 0:
                outF.write('\n'.join(pla))
                pla = []
    ...
    with open('Player.txt') as playerF, open('Action.txt') as actionF, \
        ...
        playersActionsByRound = {} # tracking in iteration for all rounds
        colsToBeWritten = {a: {r:[] for r in rounds} for a in actions}
        for i,(p,a,r) in enumerate(izip(playerF, actionF, roundF)):
        ...
        for i,p in enumerate(playerF):
            # get player's action counts for each of...
            # PFfold, PFcheck, ..., Rraise
            # write all at each 10M interval
    ...
    with open('Player.txt') as playerF, open('Table.txt') as tableF, \
        ...
        for i,(p,t,c,s) in enumerate(izip(playerF, tableF, csF, ssF)):
    ...
    with open('Player.txt') as playerF, open('GameNum.txt') as gameF, \
        ...
        ssPct = []
        playerGameSeesSD = {}
        for i,(p,g,c) in enumerate(izip(playerF, gameF, cardF)):
    ...
    with open('Player.txt') as playerF, open('Action.txt') as actionF, \
        ...
        comfold = []
        playerComFolds = {}
        for i,(p,a,v,b) in enumerate(izip(playerF, actionF, investF, bbF)):
            if a=='fold' and v!=b:
    ...
    def getAF(r):
        with open('{}BetPct.txt'.format(r)) as betF, open('{}RaisePct.txt'.format(r)) as raiseF, \
            ...
            af = []
            for i,(b,r,c) in enumerate(izip(betF, raiseF, callF)):
                af.append((b+r)/c)
                if i % 10000000 == 0:
                    outF.write('\n'.join(af) + '\n')
                    af = []
    ...
    with open('Player.txt') as playerF, open('GameNum.txt') as gameF, \
        ...
        playerFlopWins = {}
        playerFlopOpps = {}
        for i,(p,g,r,w) in enumerate(izip(playerF, gameF, roundF, winF)):
            if r!='Preflop':
